...
async function updateMR(data, branch, url, gitlabProjectId, apiKey, hasI18nFile) {
  ...
  if (mr) {
    ...
    let affectedApps = [];
    if (process.env.BH_EFFECTED_APPS) {
      affectedApps = process.env.BH_EFFECTED_APPS.split(',');
    }
    console.log('[Galaxy Parser]: Affected Apps', affectedApps);
    affectedApps.forEach(app => {
      if (app === 'novo') {
        labels.push('APP: Novo');
      } else if (app === 'admin') {
