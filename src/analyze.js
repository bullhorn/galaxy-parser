...
function deltaCheck(projectData) {
  ...
  if (diff < 0) diff *= -1;
  console.log(
    `[Galaxy Parser]: Results differ by ${diff}%${diff >= projectData.threshold ? ', posting slack message' : ', skipping slack message'}`,
  );
  ...
  try {
    ...
    if (parsed.totals.sloc) {
      projectData.lines = {
        ...
        history: projectData.lines.history.concat([[Date.now(), parsed.totals.sloc.total]]),
      };
    } else {
    ...
    projectData.coverage = {
      ...
      highest:
        parsed.totals.coverage.lines.percent > projectData.coverage.highest
          ? parsed.totals.coverage.lines.percent
          : projectData.coverage.highest,
      history: projectData.coverage.history.concat([[Date.now(), parsed.totals.coverage.lines.percent]]),
    };
    // By Module
    if (parsed.byModule) {
      ...
      for (var submodule in projectData.byModule.coverage) {
        ...
        if (projectData.byModule.coverage[submodule].coverage && projectData.byModule.coverage[submodule].coverage.highest) {
          cov.highest =
            parsed.byModule.coverage[submodule].totals.lines.percent > projectData.byModule.coverage[submodule].coverage.highest
              ? parsed.byModule.coverage[submodule].totals.lines.percent
              : projectData.byModule.coverage[submodule].coverage.highest;
        } else {
          cov.highest = parsed.byModule.coverage[submodule].totals.lines.percent;
        }
        projectData.byModule.coverage[submodule].coverage = cov;
      }
    } else {
    ...
    if (SLACK_HOOK && SLACK_CHANNEL && deltaCheck(projectData)) {
      ...
      var message = {
        text: `New results for <http://metrics:9002/#/${packageJSON.name}|${projectData.displayName}>, triggered by *${
          projectData.commit.author.name
        }*`,
